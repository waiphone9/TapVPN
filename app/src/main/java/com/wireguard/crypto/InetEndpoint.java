...
public final class InetEndpoint {
    ...
    public static InetEndpoint parse(final String endpoint) throws ParseException {
        if (FORBIDDEN_CHARACTERS.matcher(endpoint).find())
            throw new ParseException("Forbidden characters in: " + endpoint, 0);
        final URI uri;
        try {
            uri = new URI("wg://" + endpoint);
        } catch (final URISyntaxException e) {
            throw new ParseException("Invalid URI: " + endpoint + " (" + e.getMessage() + ")", 0);
        }
        if (uri.getPort() < 0 || uri.getPort() > 65535)
            throw new ParseException("Invalid port number in: " + endpoint, 0);
        try {
            InetAddress.getByName(uri.getHost());
            // ✅ Host parsed successfully as IP
            return new InetEndpoint(uri.getHost(), true, uri.getPort());
        } catch (UnknownHostException ignored) {
            // ❌ Not a numeric IP, treat as DNS hostname
