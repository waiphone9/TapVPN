...
public class RootShell {
    ...
    public int run(@Nullable final Collection<String> output, final String command)
        ...
        synchronized (lock) {
            ...
            if (markersSeen != 4)
                throw new RootShellException(RootShellException.Reason.SHELL_MARKER_COUNT_ERROR);
            if (errnoStdout != errnoStderr)
                throw new RootShellException(RootShellException.Reason.SHELL_EXIT_STATUS_READ_ERROR);
    ...
    public void start() throws IOException, RootShellException {
        if (!isExecutableInPath())
            throw new RootShellException(RootShellException.Reason.NO_ROOT_ACCESS);
        synchronized (lock) {
            if (isRunning())
                return;
            if (!localBinaryDir.isDirectory() && !localBinaryDir.mkdirs())
                throw new RootShellException(RootShellException.Reason.CREATE_BIN_DIR_ERROR);
            if (!localTemporaryDir.isDirectory() && !localTemporaryDir.mkdirs())
                throw new RootShellException(RootShellException.Reason.CREATE_TEMP_DIR_ERROR);
            try {
                ...
                try {
                    ...
                    final RootShellException rse = new RootShellException(RootShellException.Reason.NO_ROOT_ACCESS);
                    rse.initCause(e);
                    throw rse;
                }
                ...
                if (!isRunning()) {
                    ...
                    throw new RootShellException(RootShellException.Reason.SHELL_START_ERROR, process.exitValue());
                }
    ...
    public static class RootShellException extends Exception {
        ...
        // Constructor for formatted errors
        ...
        // Constructor for string-based output
        public RootShellException(final Reason reason, final CharSequence output) {
            super(reason.name());
            this.reason = reason;
            this.output = output;
            this.format = null;
